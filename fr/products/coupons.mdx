...
| `appliesToProducts` | string[] | Identifiants de **produit** auxquels le coupon est limité (optionnel) |
...
<Tabs>
  <Tab title="Pourcentage">
    ...
    ```bash
    curl -X POST https://buy.api.yabetoopay.com/v1/coupons \
...
<Warning>
  **`appliesToProducts` attend des identifiants de PRODUIT (`prod_…`).** L'éligibilité
  s'évalue en résolvant chaque article du panier vers son produit parent, puis en testant
  **ce produit-là** contre la liste.

  Rien n'attrape l'erreur — le validateur accepte n'importe quel tableau de chaînes, sans
  vérifier que les identifiants existent ni ce qu'ils désignent. Un identifiant mal formé ne
  provoque jamais d'erreur : la comparaison n'est simplement jamais vraie et **le coupon ne
  s'applique jamais**, sans le moindre message expliquant pourquoi.
</Warning>

<Note>
  Si `appliesToProducts` est défini, la réduction ne s'applique qu'aux produits listés — les
  autres articles du panier n'en bénéficient pas. Chaque article du panier est résolu vers son
  produit avant d'être comparé à la liste.
</Note>
...
```bash
curl -X POST https://buy.api.yabetoopay.com/v1/promotion-codes \
  -H "Authorization: Bearer sk_live_..." \
  -H "Content-Type: application/json" \
  -d '{
...
<Steps>
  ...
  <Step title="Produits éligibles">
    Si le coupon a `appliesToProducts`, vérifier que le panier contient au moins un article
...
| `SKUS_NOT_ELIGIBLE` | Aucun article du panier n'appartient à un produit listé dans `appliesToProducts` — le nom du code, hérité, ne reflète plus ce qu'il vérifie |
...
```bash
curl -X POST https://buy.api.yabetoopay.com/v1/promotion-codes/validate \
  -H "Authorization: Bearer sk_live_..." \
  -H "Content-Type: application/json" \
  -d '{
    "code": "SUMMER20",
...
```bash
curl -X PATCH https://buy.api.yabetoopay.com/v1/promotion-codes/promo_xxx \
  -H "Authorization: Bearer sk_live_..." \
  -H "Content-Type: application/json" \
  -d '{
    "active": false
