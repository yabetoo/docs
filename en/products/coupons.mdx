...
| `appliesToSkus` | string[] | **Product** identifiers the coupon is limited to — despite the name (optional) |
...
<Warning>
  **`appliesToSkus` expects PRODUCT identifiers (`prod_…`), not SKU identifiers.** The field
  name is a false friend: the column was renamed from `applies_to_products` to
  `applies_to_skus`, but what it holds never changed. Eligibility is evaluated by resolving
  each cart item to its parent product and testing **that product** against the list.

  Nothing catches the mistake — the validator accepts any array of strings, without checking
  that the identifiers exist or what kind of object they point at. Put `sku_…` values in
  there and the comparison is simply never true: every validation ends in
  `SKUS_NOT_ELIGIBLE` and **the coupon never applies**, with no message saying why.
</Warning>

<Note>
  If `appliesToSkus` is defined, the discount only applies to the listed products — other cart
  items don't get the discount. A cart item is resolved to its product whether it is designated
  by `skuId` or by `priceId`, so a product without any SKU can be targeted like any other.
</Note>
...
<Steps>
  ...
  <Step title="Eligible products">
    If coupon has `appliesToSkus`, verify the cart contains at least one item belonging to
    these **products**
  </Step>
...
| `SKUS_NOT_ELIGIBLE` | No item in the cart belongs to an eligible product (the code name says SKU, the check is on products) |
