...
<Info>
Use an `sk_test_` key on `https://buy.sandbox.yabetoopay.com`. The subscription's mode is the
key's: a subscription created in test stays in test for its whole life.
</Info>
